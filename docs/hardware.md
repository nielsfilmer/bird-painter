...
3. Install the app (frame side only needs `httpx`/`pillow`/`numpy`, **not** the
   ...
   .venv/bin/pip install websockets                    # so the frame can
                                                       # redraw the moment a
                                                       # bird is painted; without
                                                       # it the frame just polls
...
4. Point the frame client at the recorder and run it:
   ...
   redraws when the wall actually changed.

   At boot it looks for the recorder for `BP_FRAME_SEARCH_SECONDS` (default 60)
